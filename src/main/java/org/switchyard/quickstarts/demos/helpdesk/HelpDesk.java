...
public class HelpDesk {
    ...
    public String getGroupId() {
        final List<String> groups = USERS_GROUPS.get(_userId);
        return groups != null && groups.size() > 0 ? groups.get(0) : null;
    }

    public Map<String, String> getUsersGroups() {
        final Map<String, String> usersGroups = new LinkedHashMap<String, String>();
        for (final Map.Entry<String, List<String>> entry : USERS_GROUPS.entrySet()) {
    ...
    public void selectUser(final ValueChangeEvent vce) {
    ...
    private void fetchTasks() {
        synchronized (_userTasks) {
            _userTasks.clear();
            _userTickets.clear();
            final List<TaskSummary> tasks = _taskService.getTasksAssignedAsPotentialOwner(_userId, EN_UK);
            for (final TaskSummary task : tasks) {
                _userTasks.add(task);
                final Map<String, Object> params = _taskService.getTaskContent(task.getId());
                final Ticket ticket = (Ticket) params.get(TICKET);
    ...
    private void completeTasks() {
        synchronized (_userTasks) {
            if (_userTasks.size() > 0) {
                for (final TaskSummary task : _userTasks) {
                    _taskService.claim(task.getId(), _userId);
                    _taskService.start(task.getId(), _userId);
                    final Map<String, Object> results = new HashMap<String, Object>();
                    final Ticket ticket = _userTickets.get(task.getProcessInstanceId());
